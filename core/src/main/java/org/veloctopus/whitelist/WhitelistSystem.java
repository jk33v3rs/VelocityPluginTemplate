...
public class WhitelistSystem implements AsyncPattern {
    ...
    public CompletableFuture<Boolean> shutdownAsync() {
        ...
        return saveActiveSessionsAsync()
            .thenCompose(unused -> {
                // Shutdown scheduler
                scheduler.shutdown();
                try {
    ...
    public CompletableFuture<VerificationResult> startVerificationAsync(String discordUserId, String minecraftUsername) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            return new Object[] { platformType, cleanUsername };
        }).thenCompose(result -> {
            Object[] data = (Object[]) result;
            PlatformType platformType = (PlatformType) data[0];
            String cleanUsername = (String) data[1];
            
            // Validate username with Mojang API (async)
            return mojangApi.validateUsernameAsync(cleanUsername)
    ...
    private boolean checkRateLimiting(String discordUserId) {
        ...
        try {
            // Use a default value if cache access fails, allowing the attempt
            String currentCount = cacheLayer.getAsync(cacheKey)
                .exceptionally(ex -> "0") // Return "0" if cache fails
                .join(); // This is synchronous rate limiting - acceptable for short cache operations
